...
@Composable fun SearchScreen(vm:OweTrackViewModel,onBack:()->Unit,onPerson:(Long)->Unit){
    ...
    Scaffold(topBar={TopAppBar({Text("Search transactions")},navigationIcon={IconButton(onBack){Icon(Icons.AutoMirrored.Filled.ArrowBack,null)}})}){pad->LazyColumn(Modifier.padding(pad),contentPadding=PaddingValues(bottom=30.dp)){
